...
- **Function Calling ツール**:
  ...
  - ファイル・ディレクトリ内のテキスト検索（正規表現対応、grep的機能）
  - ファイルメタデータ取得（サイズ、更新日時、エンコーディング推定）
  - Excelシート一覧取得
  - ファイル・ディレクトリのコピー/移動
  - コマンド実行（ホワイトリスト方式、ビルド・lint・静的解析ツール対応）
  - ファイル差分比較（unified diff形式）
  - ファイル追記（ログ・レポート蓄積用）
  - Excelシート追加
  - Excel→CSVエクスポート
  - Web検索（OpenAI組み込みツール、最新情報の検索・取得）
  - Webページの取得・Markdown変換（指定URLのページ内容を取得）
- **ユーザー補助機能**:
...
- **テキスト検索**: ファイルやディレクトリ内の関数名・変数名・キーワード検索
- **コード比較**: 修正前後のファイル差分確認
- **レポート作成**: 分析結果やまとめをテキストファイルとして保存
- **ファイル検索**: ディレクトリ内の再帰的なファイル検索
- **ビルド・lint実行**: コマンド実行によるビルドや静的解析ツールの実行
- **データ変換**: ExcelシートのCSVエクスポート
- **Web情報取得**: Web検索で最新情報を取得し、Webページの内容を確認
...
| `WEB_FETCH_TIMEOUT` | No | Web取得のタイムアウト秒数 | `30` |
| `WEB_FETCH_MAX_SIZE` | No | Web取得の最大文字数 | `100000` |
...
└── src/
    ...
    ├── tools.py        # エージェントツール定義（Excel操作/ファイル操作/検索/コマンド実行/Web取得等16種）
